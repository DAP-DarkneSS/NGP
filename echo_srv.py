...
class Connect(threading.Thread):
    ...
    def run (self):
        ...
        while self.life:
            buf = sock.recv(1024)
            logging.info(self.name + " - " + buf)

            if buf == "exit":
                sock.send("bye")
                sms = self.name + " became offline."
                print(sms)
                logging.debug(sms)
                life = False
                ...
                id = id + 1
                sms = self.name + " became online!"
                filename = data_path + self.name + '.txt'
                f = open((data_path + self.name + '.txt'), 'a+')
                f.close()
                ...
                f = open((data_path + self.name + '.txt'), 'a+')
                f.close()
                sock.send("Hello, " + self.name + "!" + last_time)
